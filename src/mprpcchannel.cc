...
{
    const google::protobuf::ServiceDescriptor* sd=method->service();//根据method的service方法可以知道服务是属于哪一个对象的。
    ...
    std::string args_str;
    if(request->SerializeToString(&args_str))//序列化参数
    {
        //序列化成功
    ...
    //这里string的构造函数，将对象初始化为s的前4个字符。注意这里应该写成char*的形式，这样能够直接以二进制的形式存储。
    ...
    */
    ...
    int idx = host_data.find(":");//分割符，取出ip地址和端口号
    if(idx == -1)
    {
    ...
    //.c_str()将string类型转化为char*类型，即返回首地址
